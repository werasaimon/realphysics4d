...
void TextureReaderWriter::loadTextureFromFile(const std::string& filename, Texture2D& textureToCreate)
throw(runtime_error, invalid_argument)
{
    ...
    if (extension == "bmp")
    {
    	readBMPPicture(filename, textureToCreate);
    }
...
void TextureReaderWriter::readJPEGPicture(const std::string& filename,
                                          Texture2D& textureToCreate) throw(std::runtime_error)
 {
		...
		////	int xx;
		////	int yy;
		////	BYTE* data = LoadJPEG( filename.c_str(), &xx  , &yy );
		//    // Close the file
		//    fclose(file);
		//
...
void TextureReaderWriter::readBMPPicture( const std::string& filename , Texture2D& textureToCreate )
throw (std::runtime_error)
{
	...
	if (file == NULL)
	{
		printf( "Error: Invaild file path!  \n" );
		//return -1;
	}
	...
	for (unsigned int i=0 , j=0; i < width*height; i++)
	{
		...
		// And store it
		data[j+0] = rgb.rgbtRed; // Red component
		data[j+1] = rgb.rgbtGreen; // Green component
		data[j+2] = rgb.rgbtBlue; // Blue component
		data[j+3] = 255; // Alpha value
	...
	textureToCreate.create( width ,  height , 4 , GL_RGBA , GL_UNSIGNED_BYTE , data );
