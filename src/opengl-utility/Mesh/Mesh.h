...
enum Meshype { DEFAULT_MESH , MESH_LOAD_FILE , PRIMITIVE_MESH_BOX , PRIMITIVE_MESH_PLANE , PRIMITIVE_MESH_TRIANGLE };
...
class Mesh : public Object3D
{
    ...
    public:
        ...
        std::vector<std::vector<uint> > getIndices() const { return mIndices; }

	    const std::vector<uint>& getIndicess() const { return mIndicess; }

	    virtual void Draw() {}

	    Meshype getType() const
	    {
	    	return mType;
	    }


	    std::map<uint, Texture2D>& getTextures()
    	{
	    	return mTextures;
    	}
};
