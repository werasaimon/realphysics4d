...
struct	MinkowskiDiff
{
	...
	SIMD_INLINE Vector3 supportTransformed0(const Vector3 &direction ) const
	{
		...
		Vector3 result =  m_toshape0 * m_convexAPtr->supportWorldTransformed(m_toshape0.getOrientation().getInverse() * direction);
		return result;
	}
	...
	SIMD_INLINE Vector3 supportTransformed1(const Vector3 &direction ) const
	{
		...
		//Vector3 result = m_convexBPtr->supportWorldTransformed(direction);
		Vector3 result = m_toshape1 * m_convexBPtr->supportWorldTransformed(m_toshape1.getOrientation().getInverse() * direction);
		return result;
	}
...
//	Transform transform1 = a.getWorldTransform();
//	Transform transform2 = b.getWorldTransform();
...
//	shape.m_toshape0 =  transform1;//.getInverse() * transform2;
//	shape.m_toshape1 =  transform2;//.getOrientation().getMatrix().getInverse() *
...
//	shape.m_toshape1		=	transform2.getBasis().getInverse() * (transform1.getBasis());
//	shape.m_toshape0		=	transform1.inverseTimes(transform2);
...
int	btComputeGjkEpaPenetration2(const btCollisionDescription& colDesc, btDistanceInfo* distInfo)
{
	...
	if (res)
	{
		if ((results.status==btGjkEpaSolver3::sResults::Penetrating) || results.status==GJK::eStatus::Inside)
		{
			...
			Vector3 tmpNormalInB = results.witnesses[1]-results.witnesses[0];
